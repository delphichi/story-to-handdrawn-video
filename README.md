...
| `render-story.yml` | 手动（workflow_dispatch） | **一句话（或故事文本）→ 分镜总览 → 出图 → 视频**，扩写与出图都交给 FAL AI（需 `FAL_KEY` secret）；产物统一收进 `完成檔/<标题>-<时间戳>/` |
...
| `render-story.yml` | manual (workflow_dispatch) | **One sentence (or a story file) → storyboard sheet → illustrations → video**, with FAL AI both expanding and drawing (needs a `FAL_KEY` secret); outputs collected into `完成檔/<title>-<stamp>/` |
